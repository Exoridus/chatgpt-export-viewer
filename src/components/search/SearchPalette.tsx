...
export function SearchPalette({ open, onClose }: SearchPaletteProps) {
  ...
  const rowData = useMemo<SearchRowData>(
    () => ({ results, selectedIndex, onSelect: handleSelect }),
    ...
    >
      <div className={styles.palette}>
        ...
        <div className={styles.results}>
          {loading && <div className={styles.status}>{t.search.searching}</div>}
          ...
          {!loading && results.length > 0 && (
            <List
              ...
              rowHeight={110}
              rowProps={rowData}
              style={{ height: Math.min(results.length * 110, 500), width: '100%' }}
              tagName="ul"
            />
          )}
        </div>
...
function SearchRow({
  ...
  selectedIndex,
  style,
}: RowComponentProps<SearchRowData>) {
