...
function App() {
  ...
  useEffect(() => {
    ...
    if (notice.tone === 'error' || notice.persistent) {
      return;
    }
  ...
  useEffect(() => {
    const hasZipFiles = (transfer: DataTransfer | null) => {
      ...
      if ([...transfer.files].some(file => file.name.toLowerCase().endsWith('.zip'))) {
        return true;
      }
    ...
    const onDragEnter = (event: DragEvent) => {
      if (!hasZipFiles(event.dataTransfer)) {
        return;
      }
    ...
    const onDrop = (event: DragEvent) => {
      ...
      const files = [...(event.dataTransfer?.files ?? [])].filter(file => file.name.toLowerCase().endsWith('.zip'));
      if (!files.length) {
        return;
      }
  ...
  return (
    <div className="app-shell" data-app-shell="true">
      ...
      <div className="main-column">
        <TopBar onToggleSidebar={() => setSidebarOpen(open => !open)} onOpenUpload={() => setUploadOpen(true)} />
        <main className="content-area">
          <DeepLinkRedirect />
          <ErrorBoundary>
            <Suspense fallback={<div className="viewer-loading">{t.app.loading}</div>}>
              <Routes>
                <Route path="/" element={<HomeRoute onOpenUpload={() => setUploadOpen(true)} />} />
      ...
      {sidebarOpen ? (
        <button type="button" className="sidebar-backdrop" aria-label={t.sidebar.closeConversationList} onClick={() => setSidebarOpen(false)} />
      ) : null}
    </div>
  );
}
