...
export async function exportFullWorkingZip(
...
export async function exportFullWorkingZipFromDatabase(
  options: { onProgress?: (progress: ExportProgressState) => void } = {},
): Promise<Blob> {
...
export async function collectExportBundleData(
  ...
  const [indexCount, conversationCount, assetCount] = await Promise.all([
    db.count('index'),
  ...
  const summaryMap = Object.fromEntries(
    indexRows.map((row: ConversationSummary) => [
      row.id,
      { title: row.title, last_message_time: row.last_message_time },
    ] as const),
  )
  emitPreparing('Preparing search index and metadata…')
  const [searchBundle, extras, appFiles] = await Promise.all([
    loadSearchBundleFromDb(db, summaryMap),
    loadExtraData(db),
    fetchAppFiles(),
  ])
  return {
...
export async function buildExportArchiveBlob(
...
async function fetchAppFiles(): Promise<Record<string, Uint8Array>> {
  ...
  await Promise.all(
    Array.from(new Set(toFetch)).map(async (path) => {
      try {
...
): Promise<ViewerDB[T]['value'][]> {
  const values: ViewerDB[T]['value'][] = []
