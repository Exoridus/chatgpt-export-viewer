...
export function parseSearchPayloadFromUnknown(payload: unknown): StructuredSearchResult | null {
  ...
  const responseLength = record && typeof record.response_length === 'string' ? (record.response_length as string) : undefined;
  return {
    queries,
    meta: { responseLength },
  };
...
function extractSearchQueryList(payload: unknown): StructuredSearchQuery[] | null {
  ...
  const queries = list.map(entry => normalizeSearchEntry(entry)).filter((entry): entry is StructuredSearchQuery => Boolean(entry));
...
function normalizeSearchEntry(entry: unknown): StructuredSearchQuery | null {
  ...
  if (typeof rawQuery === 'string') {
    query = rawQuery;
  } else if (typeof rawQuery === 'number' || typeof rawQuery === 'boolean') {
