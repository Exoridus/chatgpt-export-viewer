...
export async function fetchServerGeneratedAssets(): Promise<GeneratedAsset[]> {
  try {
    ...
    return data
      .map((item) => normalizeGeneratedAsset(item))
      .filter((item): item is GeneratedAsset => item !== null)
  } catch (error) {
...
function normalizeGeneratedAsset(value: unknown): GeneratedAsset | null {
  ...
  const createdAt = normalizeGeneratedAssetTimestamp(
    row.createdAt ?? row.create_time ?? row.created_at ?? row.creation_time,
  )
  const updatedAt = normalizeGeneratedAssetTimestamp(
    row.updatedAt ?? row.update_time ?? row.updated_at ?? row.modified_at ?? row.last_modified ?? row.mtime,
  )
  return {
    path,
    fileName,
