...
export function PreferencesProvider({ children }: { children: ReactNode }) {
  const [prefs, setPrefs] = useState(localSettings.getViewerPreferences());
  ...
  useEffect(() => {
    ...
    const applyTheme = () => {
      const nextTheme = prefs.appTheme === 'system' ? (mediaQuery?.matches ? 'light' : 'dark') : prefs.appTheme;
  ...
  const value = useMemo(() => ({ viewerPreferences: prefs, setViewerPreferences, locale, t }), [prefs, setViewerPreferences, locale, t]);
