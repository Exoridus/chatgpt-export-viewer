...
import { type ImportExportSetters, useImportExportSetters } from './ImportExportContext'
...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  const [referencedAssetKeys, setReferencedAssetKeys] = useState<Set<string>>(new Set())
  ...
  const ieSettersRef = useRef<ImportExportSetters>(ieSetters)
  ...
  const setScrollPosition = useCallback((id: string, top: number) => {
    scrollPositions.set(id, top)
  }, [scrollPositions])

  const getScrollPosition = useCallback((id: string) => {
  ...
  useEffect(() => {
    ...
    async function loadLocal() {
      try {
        ...
        if (cancelled) {return}
        const [list, refKeys] = await Promise.all([loadLocalSummaries(db), findReferencedAssetKeys(db)])
        if (!cancelled) {
          setLocalIndex(list)
    ...
    async (files: File[], mode: ImportMode = 'upsert'): Promise<number> => {
      ...
      try {
        ...
        const buildConversationLabel = (completed: number) => {
          if (!conversationTotal) {return t.importer.status.saving}
        ...
        const progressiveLocalMap = new Map<string, ConversationSummary>(
          baselineLocalIndex.map((entry) => [entry.id, entry]),
        )
        ...
        const publishLocalProgress = (force = false) => {
          const now = Date.now()
          if (!force && now - lastLocalPublish < LOCAL_PUBLISH_INTERVAL) {return}
          lastLocalPublish = now
          const nextLocal = Array.from(progressiveLocalMap.values()).sort((a, b) => b.last_message_time - a.last_message_time)
          startTransition(() => {
        ...
        for (const payload of conversationsToProcess) {
          ...
          await saveConversationWithSearchData(
            db,
            payload.summary,
          ...
          const nextSummary: ConversationSummary = {
            ...payload.summary,
            source: 'local',
            pinned: previousSummary?.pinned ?? payload.summary.pinned,
          }
        ...
        setDbAllowed(true)
        const [, refKeys] = await Promise.all([updateLocalIndex(db), findReferencedAssetKeys(db)])
        setReferencedAssetKeys(refKeys)

        const assetNotice =
          savedAssets > 0 ? ` + ${savedAssets} asset${savedAssets === 1 ? '' : 's'}` : ''
        const successMessage = `${conversationTotal} conversation${conversationTotal === 1 ? '' : 's'}${assetNotice}`
  ...
  }, [dbAllowed, pinnedSet, pushNotice, serverIndex, t])
